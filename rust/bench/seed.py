...
Usage (from repo root)::
    ...
    uv run python rust/bench/seed.py --db /tmp/bench.db \\
        --runs 50000 --metrics-per-run 50 --history-points 20 \\
...
def _gen_experiments(rng: random.Random, scale: Scale, now_ms: int) -> list[int]:
    rows = []
...
def _span_content(rng: random.Random, span_id: str, name: str, span_type: str) -> str:
    """A span ``content`` JSON with an attributes map the LIKE filter can match.

    The store's span-attribute filter matches ``%"<attr>"<value>%`` against the
    raw content string, so the attribute keys/values live as plain JSON here.
    """
    ...
    return json.dumps(
        {
            ...
            "attributes": {
                ...
                "gen_ai.usage.input_tokens": rng.randint(10, 4000),
                "gen_ai.usage.output_tokens": rng.randint(10, 2000),
                "llm.temperature": round(rng.uniform(0.0, 1.5), 2),
            },
        },
        separators=(",", ":"),
    )
...
def _gen_runs(engine, rng, scale: Scale, exp_ids, now_ms, counts) -> None:
    ...
    for i in range(scale.runs):
        ...
        tags.append(
            {"key": "phase", "value": rng.choice(["train", "eval", "tune"]), "run_uuid": run_uuid}
        )
        n_tags += 2
        ...
        for mk in _METRIC_KEYS[: scale.metrics_per_run] if scale.metrics_per_run <= len(
            _METRIC_KEYS
...
def _gen_traces(engine, rng, scale: Scale, exp_ids, now_ms, counts) -> None:
    ...
    for i in range(scale.traces):
        ...
        infos.append(
            {
                ...
                "request_preview": f'{{"messages": [{{"role": "user", "content": "q{i}"}}]}}'[:1000],
            ...
            spans.append(
                {
                    ...
                    "content": _span_content(rng, span_id, name, span_type),
        ...
        if len(infos) >= _BATCH or len(spans) >= _BATCH * 2:
            flush()
    flush()
    counts.update(traces=n_traces, spans=n_spans)
...
def _gen_registry(engine, rng, scale: Scale, now_ms, counts) -> None:
    ...
    exclude.
    """
    n_models = max(1, scale.model_versions // 10)
    models, rm_tags, mvs, mv_tags = [], [], [], []
    n_mv = 0
    remaining = scale.model_versions

    for m in range(n_models):
        is_prompt = rng.random() < scale.prompt_fraction
        ...
        if is_prompt:
            rm_tags.append(
                {"workspace": "default", "name": name, "key": IS_PROMPT_TAG_KEY, "value": "true"}
            )
        else:
            rm_tags.append(
                {
        ...
        n_versions = min(remaining, rng.randint(1, 20)) if m < n_models - 1 else remaining
...
def main() -> int:
    ...
    p.add_argument("--metrics-per-run", type=int, default=50)
    p.add_argument("--history-points", type=int, default=20)
    ...
    print(f"  data generated in {time.time() - t1:.1f}s")
