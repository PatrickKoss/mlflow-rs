...
//! default-`READ` after-request tests in `auth_after_request_http.rs`. Within
//! this binary every test wants the same value, so the concurrent `set_var`
//! writes are idempotent. The env var itself is the T9.8 config seam.
